...
--[[
	...
	each source function checks the state of one type of baton
	and returns a value from 0 to 1. for binary controls, such
	as keyboard keys and gamepad buttons, they return 1 if the
	baton is held down and 0 if not. for analog controls, such
...
-- checks whether a keyboard key is down or not,
-- but it takes a scancode as an baton
...
--[[
	...
	the player object takes a configuration table and handles baton
...
--return baton
return setmetatable(baton, {__call = function(_, ...) return new(...) end})